			       ...
			       const struct nf_hook_state *state)
{
	...
	case NF_INET_PRE_ROUTING:
		...
		/* Check if we should repair the route */
			...
			kaodv_queue_enqueue_packet(skb, state->okfn, state->sk);
...
static ssize_t kaodv_read_proc(struct file *file, char *buffer, size_t length, loff_t *offset)
{  

   // len = printk("qual threshold=%d\npkts dropped=%lu\nlast qual=%d\ngateway_mode=%d\n", qual_th, pkts_dropped, qual, is_gateway);      
...
static int __init kaodv_init(void)
{
    ...
    if (!proc_create("kaodv", 0, init_net.proc_net, &fops))
