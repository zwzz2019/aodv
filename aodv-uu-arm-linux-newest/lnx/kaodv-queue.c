...
int
kaodv_queue_enqueue_packet(struct sk_buff *skb, int (*okfn) (struct net *, struct sock *,struct sk_buff *), struct sock *sk)
{
	int status = -EINVAL;
	struct kaodv_queue_entry *entry;
...
int kaodv_queue_set_verdict(int verdict, __u32 daddr)
{
	...
	if (verdict == KAODV_QUEUE_DROP) {
		...
		while (1) {
			...
			entry->okfn(&init_net, entry->sk, entry->skb);
		next:
			kfree(entry);
		}
	}
...
*/
static ssize_t kaodv_queue_get_info(struct file *file, char *buffer, size_t length, loff_t *offset)
{
	char data[128];	
	loff_t pos=*offset;

	if(pos <0 || pos >= sizeof(data))
		return 0;
	if(length > sizeof(data)-pos)
		length=sizeof(data)-pos;
...
static int init_or_cleanup(int init)
{
	...
	proc=proc_create(KAODV_QUEUE_PROC_FS_NAME, 0, init_net.proc_net, &fops_queue);
