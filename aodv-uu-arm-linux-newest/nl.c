...
void nl_init(void)
{
	...
	/////////////
	//int set=1;
	//if(setsockopt(aodvnl.sock, SOL_SOCKET, SO_NOSIGPIPE, (void *)&set, sizeof(int))<0){
		//perror("set aodvnl sock failed");
		//exit(-1);
	//}
...
int nl_send(struct nlsock *nl, struct nlmsghdr *n)
{
	...
	//struct msghdr msg =
	  //  { (void *) &peer, sizeof(peer), &iov, 1, NULL, 0, MSG_NOSIGNAL };
	// int flags = 0;
	...
	//res=sendto(nl->sock, n, n->nlmsg_len, 0, (struct sockaddr*)&peer, sizeof(peer));
